...
namespace rapidxml
{
	...
	const int print_no_char_expansion = 0x2;   //!< Printer flag mirroring the parser flag parse_no_entity_translation
    ...
    namespace internal
    {
        ...
        {
			if (flags & print_no_char_expansion)
				return copy_chars(begin, end, out);
            while (begin != end)
            {
                if (*begin == noexpand)
                {
